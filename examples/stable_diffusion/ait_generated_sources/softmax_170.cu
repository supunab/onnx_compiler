...
void softmax_170(cutlass::half_t* input,
                   ...
                   cudaStream_t stream)
{
  ...
  bool success = true;
     ...
     // For threshold K, please refer to this post: https://fb.quip.com/HCfIAbpWB0qi
      ...
      const int num_packs = (int((1024+31)/32)+7)/8;
      const int cols_per_thread = num_packs * 8;
      
        softmax_stored_locally_multi_dim<float4,cutlass::half_t,cols_per_thread><<<grid, block, 0, stream>>>((const float4*)input, (float4*)output, m, n);
