...
__global__ void
fused_elementwise_1309(uint4* output0, const uint4* input0,const uint4* input1,  int n_elements) {
...
void invoke_fused_elementwise_1309(half* output0, const half* input0,const half* input1,  int n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_1309<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
