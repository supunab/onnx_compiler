...
const int N_ELEMENTS_PER_THREAD = sizeof(half) / sizeof(half);
const int N_ELEMENTS_PER_READ = sizeof(half) / sizeof(half);
const int N_OPS_PER_THREAD = sizeof(half) / sizeof(half);
...
__global__ void
fused_elementwise_1199(half* output0,half* output1, const half* input0,const half* input1,  int n_elements) {
  ...
  half *input_tmp0 = const_cast<half*>(input0);
  
  
  input_tmp0 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ half,
                                     /*is_contiguous*/ true>(
      input_tmp0, (idx * N_ELEMENTS_PER_THREAD % (2 * 160) / (2 * 160) * (1 * 160) + idx * N_ELEMENTS_PER_THREAD % (160) / (1) * (1)) / N_ELEMENTS_PER_THREAD, 0, 0, 0);
  
    
  half tmp_i0 = *input_tmp0;
  const half* p_tmp_i0 = reinterpret_cast<const half*>(&tmp_i0);

    

  half *input_tmp1 = const_cast<half*>(input1);
  
  
  input_tmp1 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ half,
                                     /*is_contiguous*/ true>(
      input_tmp1, (idx * N_ELEMENTS_PER_THREAD % (2 * 160) / (160) * (1)) / N_ELEMENTS_PER_THREAD, 0, 0, 0);
  
    
  half tmp_i1 = *input_tmp1;
  const half* p_tmp_i1 = reinterpret_cast<const half*>(&tmp_i1);

    
  
  
  half tmp_o0;
  half* p_tmp_o0 = reinterpret_cast<half*>(&tmp_o0);
  
  half tmp_o1;
  half* p_tmp_o1 = reinterpret_cast<half*>(&tmp_o1);
  
    
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = hcos(__hmul(half(1),__hmul(p_tmp_i1[i],p_tmp_i0[i])));
p_tmp_o1[i] = hsin(__hmul(half(1),__hmul(p_tmp_i1[i],p_tmp_i0[i])));
  ...
  *output0 = tmp_o0;
  ...
  output1 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ half,
                                     /*is_contiguous*/ false>(
      output1, idx,
      160,
...
void invoke_fused_elementwise_1199(half* output0,half* output1, const half* input0,const half* input1,  int n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_1199<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<half*>(output0),reinterpret_cast<half*>(output1),
        reinterpret_cast<const half*>(input0),reinterpret_cast<const half*>(input1),
