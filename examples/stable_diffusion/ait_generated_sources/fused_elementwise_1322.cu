...
__global__ void
fused_elementwise_1322(uint4* output0, const uint4* input0,const uint4* input1,  int n_elements) {
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    half2 tmp_0 = __hadd2(p_tmp_i0[i],p_tmp_i1[i]);
p_tmp_o0[i] = tmp_0;
...
void invoke_fused_elementwise_1322(half* output0, const half* input0,const half* input1,  int n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_1322<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
