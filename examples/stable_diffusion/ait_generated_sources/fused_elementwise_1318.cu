...
__global__ void
fused_elementwise_1318(uint4* output0, const uint4* input0,const uint4* input1,  int n_elements) {
  ...
  input_tmp0 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      input_tmp0, (idx * N_ELEMENTS_PER_THREAD % (2 * 64 * 64 * 320) / (64 * 64 * 320) * (1 * 1 * 320) + idx * N_ELEMENTS_PER_THREAD % (64 * 320) / (64 * 320) * (1 * 320) + idx * N_ELEMENTS_PER_THREAD % (320) / (1) * (1)) / N_ELEMENTS_PER_THREAD, 0, 0, 0);
...
void invoke_fused_elementwise_1318(half* output0, const half* input0,const half* input1,  int n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_1318<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
