...
void softmax_455(cutlass::half_t* input,
                   ...
                   cudaStream_t stream)
{
  ...
  bool success = true;
      ...
      dim3 block(thread_group_width, thread_group_per_block);
        ...
        softmax_stored_locally_multi_dim<float4,cutlass::half_t,8><<<grid, block, 0, stream>>>( (const float4*)input, (float4*)output, m, n);
