...
if __name__ == "__main__":
    ...
    latent_model_input_original = np.random.rand(batch_size, input_channels, hh, ww).astype(np.float16)
    # AIT expects in a permuted order, hence permute
    latent_model_input = latent_model_input_original.transpose(0, 2, 3, 1)
    ...
    output_ort = ort.OrtValue.ortvalue_from_shape_and_type([batch_size, input_channels, hh, ww], np.float16, "cuda", 0)
