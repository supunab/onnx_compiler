...
__global__ void
fused_elementwise_1198(uint4* output0, const uint4* input0,  int n_elements) {
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = h2exp(__hmul2(__hmul2(half2(-9.210340371976184,-9.210340371976184),p_tmp_i0[i]),half2(0.00625,0.00625)));
...
void invoke_fused_elementwise_1198(half* output0, const half* input0,  int n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_1198<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
